...
export default function Order() {
  ...
  return (
    <div style={{ minHeight: "70vh" }} className='flex h-fit bg-gray-100'>
      ...
      <div className='flex flex-col gap-3 ml-11 mt-4'>
        ...
        {order?.length === 0 ? (
          <div className="flex flex-col items-center">
            <p className="text-gray-600 text-lg mb-4">No orders. Please order something!</p>
            <Link to="/" className="text-blue-500 hover:text-blue-700 text-lg">Back to Home</Link>
          </div>
        ) : (
          <div>
            {order?.flat().map((item) => (
